...
if [ ! -d /lib/modules/2.6.31-23-generic ]; then
  echo "No /lib/modules/2.6.31-23-generic"
  exit 1
fi
if [ ! -d /lib/modules/2.6.31-23-generic/extra ]; then
  mkdir /lib/modules/2.6.31-23-generic/extra
  if [ "$?" != 0 ]; then
	exit 1
  fi
fi
...
read p3PARM
	if [ "$p3PARM" = "p" ] || [ "$p3PARM" = "P" ]; then
		cp p3primary.ko /lib/modules/2.6.31-23-generic/extra/.
		cp p3sysprimary /usr/sbin/.
		cp p3primary.conf /etc/p3/.
	elif [ "$p3PARM" = "s" ] || [ "$p3PARM" = "S" ]; then
		cp p3secondary.ko /lib/modules/2.6.31-23-generic/extra
