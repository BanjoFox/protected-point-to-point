...
int init_kernel_comm()
{
...
#ifndef _p3_SECONDARY
	p3utils->anchor->kserv = (p3key_serv *) mmap (NULL, RAMDISK_SZ, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
	if (p3utils->anchor->kserv == (p3key_serv *) MAP_FAILED) {
		sprintf(p3buf, "init_kernel_comm: Failed to allocate p3 key server structure: %s\n",
				strerror(errno));
		p3errmsg(p3MSG_CRIT, p3buf);
		close(fd);
		stat = -1;
	...
	p3utils->anchor->kserv->cbuf_sz = RAMDISK_SZ;
...
int init_raw_socket(int ipver)
{
	...
	if(setsockopt(sd, IPPROTO_IP, IP_HDRINCL, &sopt, sizeof(sopt)) < 0) {
		sprintf (p3buf, "init_raw_socket: Failed to set Raw socket options:\
 %s\n", strerror (errno));
		p3errmsg (p3MSG_CRIT, p3buf);
		stat = -1;
		goto out;
...
p3errmsg(p3MSG_DEBUG, "Send Raw packet\n");
	if (sendto(sd, rawmsg, len, 0, (struct sockaddr *)&sa4, sizeof(sa4)) < 0) {
		sprintf (p3buf, "init_raw_socket: Failed to send packet to Raw socket:\
 %s\n", strerror (errno));
		p3errmsg (p3MSG_CRIT, p3buf);
		stat = -1;
		goto out;
